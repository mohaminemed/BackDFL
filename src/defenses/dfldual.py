...
def model_inversion_generate_dummy_dataset(
    ...
    Fixes "can't optimize a non-leaf Tensor" by ensuring model.parameters()
    are leaf tensors with requires_grad=True before autograd.grad is used.
    """
    ...
    # Ensure parameters are new leaf nn.Parameter objects
    for name, p in model.named_parameters():
      new_p = nn.Parameter(p.detach().clone().to(device), requires_grad=True)
      # assign to correct submodule using attribute traversal
      parts = name.split('.')
      module = model
      for part in parts[:-1]:
    ...
    # flatten target gradient vector (move to device)
        ...
        for s in range(num_samples_per_class):
            ...
            for step in range(steps):
                ...
                if len(input_shape) > 1:
                    inp = x.view((1,) + input_shape)
                else:
                    inp = x
                logits = model(inp)
                label = torch.tensor([lab], device=device)
                loss = loss_fn(logits, label)

                # compute grads w.r.t. model parameters
                grads = torch.autograd.grad(loss, list(model.parameters()), create_graph=False, retain_graph=False)
                # flatten grads into vector (CPU)
                grad_vec = torch.cat([g.detach().cpu().flatten() for g in grads], dim=0).to(device)

                # matching objective to the target equivalent gradient
                diff = grad_vec - s_t * tgt_vec
                obj = torch.norm(diff) ** 2
                obj.backward()   # this backprops to x because model params are leaf with requires_grad
                opt.step()

            outputs[lab].append(x.detach().cpu().numpy().reshape(-1))

    final = {}
    for lab, arrs in outputs.items():
        final[lab] = np.stack(arrs, axis=0)
    return final
...
class DFLDualServer(FedAvgAggregator):
    ...
    Configuration (config dict):
      ...
      - C1 (float) default 1e6
      - C2 (float) default 10.0
      - dummy_samples_per_class (int) default 5
      - inversion_steps (int) default 100
      - inversion_lr (float) default 0.05
      - inversion_s (float) default 1.0
      - use_inversion (bool) default True
      - fast_mode (bool) default True  -> reduces inversion cost defaults
    ...
    def __init__(self, model: nn.Module, testloader: nn.Module = None, device: Optional[torch.device] = None, config: Optional[Dict] = None):
        ...
        print(f"Initialized DFLDualServer(alpha={self.alpha}, C1={self.C1}, use_inversion={self.use_inversion}, fast_mode={self.fast_mode})")
    ...
    def _entry_model_state(self, entry: Any) -> Dict[str, torch.Tensor]:
        if isinstance(entry, dict) and 'model' in entry:
            return entry['model']
        elif isinstance(entry, dict) and all(isinstance(v, torch.Tensor) for v in entry.values()):
    ...
    def _compute_equivalent_gradient(self, entry: Any, round_idx: int) -> Dict[str, torch.Tensor]:
        ...
        Returns a dict (state-dict shaped) on CPU.
        """
        ...
        if isinstance(entry, dict) and entry.get('prev_model') is not None:
            ...
            prev_state = self.prev_global_state if self.prev_global_state is not None else {k: torch.zeros_like(v) for k, v in model_state.items()}

        E = float(entry.get('E', self.default_local_epochs)) if isinstance(entry, dict) else self.default_local_epochs
        B = float(entry.get('B', self.default_batch_size)) if isinstance(entry, dict) else self.default_batch_size
        local_size = float(entry.get('local_size', entry.get('local_size', 1.0) if isinstance(entry, dict) else 1.0) ) if isinstance(entry, dict) else 1.0
    ...
    def _compute_dual_distance_matrix(self, models: List[Dict[str, torch.Tensor]], dummy_datasets: List[Optional[Dict[int, np.ndarray]]]) -> np.ndarray:
        n = len(models)
    ...
    def aggregate(self, current_round: int) -> Dict[str, torch.Tensor]:
        ...
        if self.use_inversion:
            for idx, entry in enumerate(self.received_params):
                try:
                    ...
                    # prepare a model copy with entry weights loaded (use server model architecture)
                    try:
                        model_copy = copy.deepcopy(self.model)
                        model_copy.load_state_dict(self._entry_model_state(entry))
                    except Exception as exc:
                        print(f"[DFL-Dual] Warning: cannot load entry model into server architecture for idx={idx}. Skipping inversion. Exc: {exc}")
                    ...
                    dd = model_inversion_generate_dummy_dataset(
                        ...
                        s_t=(self.inversion_s ** current_round) if self.inversion_s != 1.0 else 1.0
                    )
        ...
        #print("[DFLDUAL] Computing Dual distance matrix D_ij...")
        ...
        # Stage 1: for each j, cluster other clients (k != j) into two groups by median D_kj and pick M*_j by trust
        n = len(models)
        ...
        # Stage 2: cluster r_j into two groups by median and pick group with higher trust
        ...
        # Aggregate: arithmetic mean (paper) OR weighted if you prefer (use 'weighted_aggregation' config)
        weighted_agg = bool(getattr(self, 'weighted_aggregation', False))
