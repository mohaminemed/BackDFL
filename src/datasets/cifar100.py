...
class CIFAR100Adapter(DatasetAdapter):
    def __init__(self, root: str = "data", train: bool = True, download: bool = True, transform=None):
        ...
        if transform is None:
            transform = get_transforms("cifar100", train=train)
        self.transform = transform
        super().__init__(root=root, train=train, download=download, transform=transform)   
    
    def load_dataset(self) -> None:
        self._dataset = CIFAR100(root=self.root, train=self.train, transform=self.transform, download=self.download)

    def get_test_loader(self, batch_size = 256, shuffle = False):
        test_set = CIFAR100(root=self.root, train=False , download=True, transform=get_transforms(dataset_name="cifar100", train=False))
